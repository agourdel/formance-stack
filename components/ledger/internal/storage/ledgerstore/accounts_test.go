...
func TestGetAccounts(t *testing.T) {
	...
	//old_oot := now.Add(-1*time.Minute)
	futur_oot := now.Add(200*time.Millisecond)

	pit2 := now.Add(4*time.Minute)
	oot2 := now.Add(-1 * time.Minute)
		...
		ledger.ChainLogs(
			...
			ledger.NewTransactionLog(
				ledger.NewTransaction().
					...
					WithIDUint64(3),
				map[string]metadata.Metadata{},
			).WithDate(now.Add(300*time.Millisecond)),
			ledger.NewTransactionLog(
				ledger.NewTransaction().
					WithPostings(ledger.NewPosting("world", "account:1", "USD", big.NewInt(150))).
					WithDate(now.Add(-4*time.Minute)).
					WithIDUint64(4),
				map[string]metadata.Metadata{},
			).WithDate(now.Add(400*time.Millisecond)),
	...
	t.Run("list with volumes", func(t *testing.T) {
		...
		require.Equal(t, ledger.VolumesByAssets{
			"USD": ledger.NewVolumesInt64(350, 50),
	...
	t.Run("list with effective volumes using PIT", func(t *testing.T) {
		...
		require.Len(t, accounts.Data, 1)
		require.Equal(t, ledger.VolumesByAssets{
			"USD": ledger.NewVolumesInt64(250, 0),
		}, accounts.Data[0].EffectiveVolumes)
	})

	t.Run("list with volumes using OOT", func(t *testing.T) {
		t.Parallel()
		accounts, err := store.GetAccountsWithVolumes(ctx, NewGetAccountsQuery(NewPaginatedQueryOptions(PITFilterWithVolumes{
			PITFilter: PITFilter{
				OOT: &futur_oot,
		...
		require.Len(t, accounts.Data, 1)
		require.Equal(t, ledger.VolumesByAssets{
			"USD": ledger.NewVolumesInt64(150, 50),
		}, accounts.Data[0].Volumes)
	})

	t.Run("list with volumes using OOT2", func(t *testing.T) {
		t.Parallel()
		accounts, err := store.GetAccountsWithVolumes(ctx, NewGetAccountsQuery(NewPaginatedQueryOptions(PITFilterWithVolumes{
			PITFilter: PITFilter{
				OOT: &oot2,
	...
	t.Run("list with effective volumes using  OOT", func(t *testing.T) {
		t.Parallel()
		accounts, err := store.GetAccountsWithVolumes(ctx, NewGetAccountsQuery(NewPaginatedQueryOptions(PITFilterWithVolumes{
			PITFilter: PITFilter{
				OOT: &now,
	...
	t.Run("list with volumes using PIT and OOT", func(t *testing.T) {
		t.Parallel()
		accounts, err := store.GetAccountsWithVolumes(ctx, NewGetAccountsQuery(NewPaginatedQueryOptions(PITFilterWithVolumes{
			PITFilter: PITFilter{
				OOT: &now,
	...
	t.Run("list with effective volumes using PIT and OOT", func(t *testing.T) {
		t.Parallel()
		accounts, err := store.GetAccountsWithVolumes(ctx, NewGetAccountsQuery(NewPaginatedQueryOptions(PITFilterWithVolumes{
			PITFilter: PITFilter{
				OOT: &oot2,
