...
func (store *Store) V2GetAggregatedBalances(ctx context.Context, q GetAggregatedBalanceQuery) (ledger.BalancesByAssetsByAccounts, error){
	var (
		needMetadata bool
		subQuery     string
		args         []any
	...
	type Temp struct {
		Aggregated ledger.VolumesByAssetsByAccounts `bun:"aggregated,type:jsonb"`
	}
	ret, err := fetch[*Temp](store, false, ctx,
		func(selectQuery *bun.SelectQuery) *bun.SelectQuery {
			pitColumn := "effective_date"
			...
			formatAccountStr := "'*'"
			if (groupLvl > 0){
				moves = moves.ColumnExpr(fmt.Sprintf(`(array_to_string((string_to_array(account_address, ':'))[1:LEAST(array_length(string_to_array(account_address, ':'),1),%d)],':')) as segment_address`, groupLvl))
				formatAccountStr = "moves.segment_address"
			}

			formatVolumeStr := fmt.Sprintf("volumes_to_jsonb((moves.asset, (sum((moves.%s).inputs), sum((moves.%s).outputs))::volumes))", volumesColumn, volumesColumn)

			formatReturnObjetStr := fmt.Sprintf("jsonb_build_object(%s, %s) as aggregated", formatAccountStr, formatVolumeStr)

			
			asJsonb := selectQuery.NewSelect().
				TableExpr("moves").
				ColumnExpr(formatReturnObjetStr)
				
			if (groupLvl > 0) {
				asJsonb = asJsonb.Group("moves.segment_address")
			} 

			asJsonb = asJsonb.Group("moves.asset")
	...
	if errors.Is(err, sqlutils.ErrNotFound) {
		return ledger.BalancesByAssetsByAccounts{}, nil
	}
